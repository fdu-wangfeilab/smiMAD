...
def train(adata_omics1,adata_omics2,save_path='',n_cluster=5,Epoch = 5000,lr = 0.001,weight_decay = 0.01,device='cuda:0',other_type='adt'):
    common = 'feat' in adata_omics1.obsm
    if not common:
        ...
        RNA=torch.tensor(adata_omics1.obsm['feat']).float().to(device)
        ADT=torch.tensor(adata_omics2.obsm['feat']).float().to(device)
    ...
    if not common:
        if other_type=='adt':
            model=smiMAD(RNA.shape[1],ADT.shape[1]).to(device)
        else:
            model=smiMAD(RNA.shape[1],ATAC.shape[1],modality_type='atac').to(device)
    else:
        model=smiMAD(RNA.shape[1],ADT.shape[1],common=True).to(device)
    ...
    for epoch in progress_bar:
        ...
        if not common:
            ...
            modality1_reconstruction, modality2_reconstruction,spatial_specific1,spatial_shared1,feature_shared1,feature_specific1,spatial_specific2,spatial_shared2,feature_shared2,feature_specific2,fused_latent,_,_,_=model(RNA,ADT,spatial_adj,adj_feature_f1,adj_feature_f2)
            rec_1 = adt_loss(modality1_reconstruction, RNA)
            rec_2 = adt_loss(modality2_reconstruction, ADT)
    ...
    if not common:
        ...
        with torch.no_grad():
            _, _, _, _, _, _,_,_,_,_,fused_latent,attention_weights1,attention_weights2,attention_weights_fusion=model(RNA,ADT,spatial_adj,adj_feature_f1,adj_feature_f2)
